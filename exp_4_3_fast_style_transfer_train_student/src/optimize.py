...
def optimize(content_targets, style_target, content_weight, style_weight,
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'), tf.Session() as sess:
        ...
        if slow:
            ...
            preds = transform.net(X_content/255.0, type=type)

        # TODO：preds_pre 输入到特征提取网络，并将此时得到的特征提取网络传递给 net
        net = vgg.net(vgg_path, preds_pre)

        # TODO：计算内容损失 content_loss, 风格损失 style_loss, 全变分正则化项 tv_loss, 损失函数 loss
        content_loss, style_loss, tv_loss, loss = loss_function(net, content_features, style_features, content_weight, style_weight, tv_weight, preds_pre, batch_size)
